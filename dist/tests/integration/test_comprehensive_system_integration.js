...
const MessageSystem_1 = require("../../src/core/messaging/MessageSystem");
...
describe('Comprehensive System Integration', () => {
    ...
    before(async () => {
        ...
        messageSystem = MessageSystem_1.MessageSystem.getInstance();
        monitoringSystem = MonitoringSystem_1.MonitoringSystem.getInstance();
        storageSystem = StorageSystem_1.StorageSystem.getInstance();
        // Mock OpenAI client
        mockOpenAIClient = {
        ...
        // Initialize agents
        analysisAgent = new AnalysisAgent_1.AnalysisAgent('Integration Analysis Agent', {
        ...
        creativeWritingAgent = new CreativeWritingAgent_1.CreativeWritingAgent('Integration Writing Agent', {
        ...
        dataAnalysisAgent = new DataAnalysisAgent_1.DataAnalysisAgent('Integration Data Analyst', {
            id: 'integration-data',
        ...
        developmentAgent = new DevelopmentAgent_1.DevelopmentAgent('Integration Developer', {
        ...
        financeAgent = new FinanceAgent_1.FinanceAgent('Integration Finance Analyst', {
    ...
    describe('Core System Integration', () => {
        it('should integrate Event System with Monitoring', async () => {
            ...
            (0, chai_1.expect)(newEventCount).to.be.at.least(eventCount + 2);
        });
        it('should integrate Message System with Storage', async () => {
            ...
            (0, chai_1.expect)(storedMessage).to.not.be.undefined;
            (0, chai_1.expect)(storedMessage.id).to.equal('test-msg-123');
            (0, chai_1.expect)(storedMessage.content).to.equal('Integration test message');
        });
        it('should integrate Monitoring with Storage', async () => {
            ...
            if (storedMetrics) {
                (0, chai_1.expect)(storedMetrics.integrationTest).to.equal(42);
            }
        });
    });
    describe('Agent Integration with Core Systems', () => {
        it('should integrate agents with Event System', async () => {
            ...
            (0, chai_1.expect)(eventReceived).to.be.true;
            // Cleanup
        ...
        it('should integrate agents with Monitoring System', async () => {
            ...
            if (typeof monitoringSystem.getMetrics === 'function') {
                ...
                (0, chai_1.expect)(metrics['agent.analysis.completed']).to.equal(1);
                (0, chai_1.expect)(metrics['agent.writing.completed']).to.equal(1);
                (0, chai_1.expect)(metrics['agent.data.completed']).to.equal(1);
            }
        });
        it('should integrate agents with Storage System', async () => {
            ...
            (0, chai_1.expect)(storedAnalysis).to.not.be.undefined;
            (0, chai_1.expect)(storedWriting).to.not.be.undefined;
        });
    });
    describe('Cross-Agent Communication', () => {
        it('should enable communication between agents', async () => {
            ...
            (0, chai_1.expect)(analysisReceived).to.be.true;
            (0, chai_1.expect)(writingReceived).to.be.true;
            // Cleanup
            analysisCleanup();
            writingCleanup();
        });
        it('should handle agent collaboration workflows', async () => {
            ...
            const reportResult = await creativeWritingAgent.generateContent('Create a report based on the analysis and visualization', { format: 'blog-post' });
            ...
            (0, chai_1.expect)(storedAnalysis).to.not.be.undefined;
            (0, chai_1.expect)(storedViz).to.not.be.undefined;
            (0, chai_1.expect)(storedReport).to.not.be.undefined;
            (0, chai_1.expect)(storedReport.content).to.be.a('string');
            (0, chai_1.expect)(storedReport.content.length).to.be.greaterThan(0);
        });
    });
    describe('Error Handling and Resilience', () => {
        ...
        it('should maintain system stability during component failures', async () => {
            ...
            (0, chai_1.expect)(stableValue.value).to.equal('stable');
            ...
            (0, chai_1.expect)(stillStableValue.value).to.equal('still-stable');
    ...
    describe('Performance and Scalability', () => {
        it('should handle concurrent agent operations', async () => {
            ...
            (0, chai_1.expect)(duration).to.be.below(30000); // Less than 30 seconds
        });
        it('should handle high-volume event processing', async () => {
            ...
            const duration = endTime - startTime;
            (0, chai_1.expect)(duration).to.be.below(2000); // Less than 2 seconds
            // Cleanup
            eventSystem.unsubscribe(subscriptionId);
        });
        it('should handle large data storage and retrieval', async () => {
            ...
            (0, chai_1.expect)(storeDuration).to.be.below(5000); // Less than 5 seconds
            // Retrieve large data
            const retrieveStartTime = Date.now();
            const retrievedData = await storageSystem.get('test.large-data');
            const retrieveEndTime = Date.now();
            // Should retrieve correctly
            (0, chai_1.expect)(retrievedData).to.deep.equal(largeData);
            // Should retrieve quickly
            const retrieveDuration = retrieveEndTime - retrieveStartTime;
            (0, chai_1.expect)(retrieveDuration).to.be.below(1000); // Less than 1 second
        });
    });
    describe('End-to-End Workflow Integration', () => {
        it('should execute complete multi-agent workflow', async () => {
            ...
            const codeResult = await developmentAgent.generateCode('Create a dashboard component to display sales and customer data', { language: 'TypeScript', includeComments: true });
            await storageSystem.set(`workflow.${workflowId}.code`, codeResult);
            // Step 5: Writing Agent creates report
            const reportResult = await creativeWritingAgent.generateContent(`Create a comprehensive business report based on sales analysis: ${analysisResult.summary}`, { format: 'blog-post', tone: 'professional' });
            await storageSystem.set(`workflow.${workflowId}.report`, reportResult);
            ...
            (0, chai_1.expect)(workflowResults.analysis).to.not.be.undefined;
            (0, chai_1.expect)(workflowResults.visualization).to.not.be.undefined;
            (0, chai_1.expect)(workflowResults.code).to.not.be.undefined;
            (0, chai_1.expect)(workflowResults.report).to.not.be.undefined;
            (0, chai_1.expect)(workflowResults.finance).to.not.be.undefined;
            // Results should have expected structure
            (0, chai_1.expect)(workflowResults.analysis.summary).to.be.a('string');
            (0, chai_1.expect)(workflowResults.visualization.visualizationType).to.be.a('string');
            (0, chai_1.expect)(workflowResults.code.code).to.be.a('string');
            (0, chai_1.expect)(workflowResults.report.content).to.be.a('string');
            (0, chai_1.expect)(workflowResults.finance.summary).to.be.a('string');
        });
    });
    describe('System Monitoring and Observability', () => {
        it('should provide comprehensive monitoring data', async () => {
            ...
            (0, chai_1.expect)(healthResult.status).to.equal('healthy');
            // Get metrics if available
            if (typeof monitoringSystem.getMetrics === 'function') {
                const metrics = monitoringSystem.getMetrics();
                (0, chai_1.expect)(metrics).to.be.an('object');
                (0, chai_1.expect)(metrics['agent.analysis.operations']).to.equal(1);
                (0, chai_1.expect)(metrics['agent.writing.operations']).to.equal(1);
                (0, chai_1.expect)(metrics['agent.data.operations']).to.equal(1);
            }
            // Cleanup
    ...
    describe('Data Persistence and Recovery', () => {
        it('should persist data across system restarts', async () => {
            ...
            (0, chai_1.expect)(storedData).to.not.be.undefined;
            (0, chai_1.expect)(storedData.data).to.equal('should survive restart');
            // Simulate data recovery (re-initialization)
            const recoveredData = await storageSystem.get('test.persistence');
            (0, chai_1.expect)(recoveredData).to.not.be.undefined;
            (0, chai_1.expect)(recoveredData.data).to.equal('should survive restart');
        });
        it('should handle data corruption recovery', async () => {
            ...
            (0, chai_1.expect)(retrievedData).to.deep.equal(testData);
            (0, chai_1.expect)(retrievedData.string).to.equal('test');
            (0, chai_1.expect)(retrievedData.number).to.equal(42);
            (0, chai_1.expect)(retrievedData.object.nested).to.equal('value');
            (0, chai_1.expect)(retrievedData.array).to.deep.equal([1, 2, 3]);
        });
    });
});
//# sourceMappingURL=test_comprehensive_system_integration.js.map