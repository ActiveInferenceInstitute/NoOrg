...
describe('MessageSystem', () => {
    ...
    describe('Singleton Pattern', () => {
        it('should return the same instance', () => {
            const instance1 = MessageSystem_1.MessageSystem.getInstance();
            const instance2 = MessageSystem_1.MessageSystem.getInstance();
    ...
    describe('Message Sending and Receiving', () => {
        it('should send and receive messages', async () => {
            ...
            (0, chai_1.expect)(receivedMessage.type).to.equal(messageType);
            (0, chai_1.expect)(receivedMessage.body).to.deep.equal(messageBody);
            (0, chai_1.expect)(receivedMessage.metadata).to.have.property('correlationId');
            (0, chai_1.expect)(sentMessage.id).to.equal('msg-123');
        });
        it('should handle message responses', async () => {
            ...
            (0, chai_1.expect)(responseReceived).to.not.be.null;
            (0, chai_1.expect)(responseReceived.type).to.equal(responseType);
            (0, chai_1.expect)(responseReceived.body.result).to.equal('success');
            (0, chai_1.expect)(responseReceived.metadata?.correlationId).to.equal('corr-789');
        });
        it('should support request-response pattern', async () => {
            ...
            (0, chai_1.expect)(response).to.not.be.null;
            (0, chai_1.expect)(response.type).to.equal('test.response');
            (0, chai_1.expect)(response.body.processed).to.be.true;
            (0, chai_1.expect)(response.body.originalMessage.data).to.equal('test');
        });
    });
    describe('Message Routing', () => {
        it('should route messages to correct handlers', async () => {
            ...
            // Wait for processing
            await new Promise(resolve => setTimeout(resolve, 50));
            (0, chai_1.expect)(topic1Received).to.be.true;
            (0, chai_1.expect)(topic2Received).to.be.true;
        });
        it('should support broadcast messages', async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 100));
            (0, chai_1.expect)(receivedCount).to.equal(receivers.length);
        });
    });
    ...
    describe('Performance', () => {
        it('should handle high message volume', async () => {
            ...
            (0, chai_1.expect)(receivedCount).to.equal(messageCount);
            (0, chai_1.expect)(endTime - startTime).to.be.below(2000); // Should complete in less than 2 seconds
        });
    });
    describe('Message Filtering and Transformation', () => {
        it('should support message filtering', async () => {
            const filterType = 'test.filter';
            let filteredMessage = null;
            // Register filter
            ...
            (0, chai_1.expect)(filteredMessage).to.not.be.null;
            (0, chai_1.expect)(filteredMessage.body.filterMe).to.be.false;
        });
        it('should support message transformation', async () => {
            ...
            // Wait for processing
            await new Promise(resolve => setTimeout(resolve, 50));
            (0, chai_1.expect)(transformedMessage).to.not.be.null;
            (0, chai_1.expect)(transformedMessage.body.transformed).to.be.true;
            (0, chai_1.expect)(transformedMessage.body.originalValue).to.equal(42);
            (0, chai_1.expect)(transformedMessage.body.doubledValue).to.equal(84);
        });
    });
    describe('Message Persistence', () => {
        it('should support message queuing', async () => {
            ...
            (0, chai_1.expect)(processedCount).to.equal(2);
        });
    });
...
//# sourceMappingURL=test_message_system.js.map