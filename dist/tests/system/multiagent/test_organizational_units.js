...
describe('Organizational Unit-Based Multiagent Composition', function () {
    this.timeout(test_helpers_1.TEST_TIMEOUT);
    // Test fixture
    let fixture;
    ...
    it('should compose agents based on organizational units', async function () {
        ...
        (0, chai_1.expect)(units.length).to.be.greaterThan(0, 'No organizational units found');
        ...
        for (const unit of testUnits) {
            ...
            const agent = (0, test_helpers_1.createTestAgent)(unit, capabilities, {
                name: `${unit} Agent`,
        ...
        (0, chai_1.expect)(agentIds.length).to.equal(agents.length, 'Not all agents were registered');
        ...
        const coordinatorAgent = (0, test_helpers_1.createTestAgent)('Coordinator', ['coordination', 'oversight', 'planning'], {
        ...
        for (const task of [coordinationTask, ...workflowTasks]) {
            const taskObj = await fixture.taskManager.getTask(task);
            (0, chai_1.expect)(taskObj.status).to.equal('completed', `Task ${task} was not completed`);
        }
        ...
        (0, chai_1.expect)(state.status).to.equal('completed', 'Composition was not marked as completed');
    });
    /**
    ...
    it('should integrate LLM capabilities with organizational units', async function () {
        ...
        const scenario = {
            ...
            agents: [
                (0, test_helpers_1.createTestAgent)('Research', ['research', 'analysis'], {
                    name: 'Research Unit Agent',
                    metadata: { unit: 'Research', test: testId }
                }),
                (0, test_helpers_1.createTestAgent)('Development', ['coding', 'testing'], {
                    name: 'Development Unit Agent',
                    metadata: { unit: 'Development', test: testId }
                }),
                (0, test_helpers_1.createTestAgent)('Operations', ['deployment', 'monitoring'], {
            ...
            tasks: [
                (0, test_helpers_1.createTestTask)('research', 'Research the best practices for multiagent systems', {
                    title: 'Research Task',
                    metadata: { unit: 'Research', test: testId }
                }),
                (0, test_helpers_1.createTestTask)('development', 'Design a system architecture for multiagent deployment', {
                    title: 'Development Task',
                    metadata: { unit: 'Development', test: testId }
                }),
                (0, test_helpers_1.createTestTask)('operations', 'Plan the deployment of multiagent systems', {
        ...
        // Set up the scenario
        const scenarioSetup = await (0, test_helpers_1.setupTestScenario)(scenario);
        // Execute the tasks
        ...
        for (const taskId of scenarioSetup.taskIds) {
            const task = await scenarioSetup.fixture.taskManager.getTask(taskId);
            (0, chai_1.expect)(task.status).to.equal('completed', `Task ${taskId} was not completed`);
            (0, chai_1.expect)(task.result).to.exist;
            (0, chai_1.expect)(task.result.success).to.be.true;
        }
        // Update scenario state
        await scenarioSetup.fixture.sharedStateManager.setState(`test.${testId}.llm_scenario.status`, 'completed');
        ...
        (0, chai_1.expect)(state.status).to.equal('completed', 'LLM scenario was not marked as completed');
    });
});
//# sourceMappingURL=test_organizational_units.js.map