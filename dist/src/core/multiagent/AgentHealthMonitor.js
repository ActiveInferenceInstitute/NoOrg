...
class AgentHealthMonitor {
    ...
    constructor(agentRegistry) {
        Object.defineProperty(this, "healthChecks", {
            ...
            writable: true,
            value: new Map()
        }); // agentId -> checkId -> HealthCheck
        Object.defineProperty(this, "healthStatuses", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "healthThresholds", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.logger = new Logger_1.Logger('AgentHealthMonitor');
        // Default thresholds
     ...
     */
    async registerHealthCheck(agentId, check) {
     ...
     */
    async unregisterHealthCheck(agentId, checkId) {
     ...
     */
    async startHealthCheck(agentId, checkId) {
        const checks = this.healthChecks.get(agentId);
        if (!checks)
            return;
     ...
     */
    async stopHealthCheck(agentId, checkId) {
        const intervalKey = `${agentId}:${checkId}`;
     ...
     */
    async performHealthCheck(agentId, checkId) {
        const checks = this.healthChecks.get(agentId);
        if (!checks) {
     ...
     */
    async updateHealthStatus(agentId, checkId, result) {
     ...
     */
    async calculateMetrics(agentId) {
     ...
     */
    async generateHealthReport(agentId) {
        ...
        else if (status.metrics.successRate < this.healthThresholds.minSuccessRate ||
     ...
     * Register default health checks
     */
    async registerDefaultHealthChecks(agentId) {
     ...
     */
    async resolveIssue(agentId, issueId) {
        const status = this.healthStatuses.get(agentId);
        if (!status)
            return;
...
//# sourceMappingURL=AgentHealthMonitor.js.map