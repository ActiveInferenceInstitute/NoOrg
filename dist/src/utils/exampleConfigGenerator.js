...
const ProjectConfigGenerator_1 = require("./ProjectConfigGenerator");
...
async function main() {
    ...
    const configGenerator = ProjectConfigGenerator_1.ProjectConfigGenerator.getInstance(apiKey);
    ...
    for (const domain of domains) {
        ...
        fs.writeFileSync(path.join(outputDir, fileName), JSON.stringify(autoConfig, null, 2));
    }
    // Example of using the config with a custom objective
    console.log('\n=== Custom Objective Example ===');
    const customConfig = await configGenerator.generateConfig('Smart Agriculture', 'Develop a drone-based crop monitoring system for precision farming', {
    ...
    fs.writeFileSync(path.join(outputDir, 'smart-agriculture-custom.json'), JSON.stringify(customConfig, null, 2));
...
async function runHybridAgentWorkflow(options = {}) {
  ...
  `;
    ...
    fs.writeFileSync(path.join(outputDir, 'integration-example.ts'), exampleCode);
}
// Run the example
main().catch(err => {
...
//# sourceMappingURL=exampleConfigGenerator.js.map