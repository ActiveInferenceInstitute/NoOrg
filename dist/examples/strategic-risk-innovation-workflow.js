...
exports.OrganizationalWorkflowManager = void 0;
exports.runStrategicInnovationWorkflow = runStrategicInnovationWorkflow;
...
const OrganizationalStructureManager_1 = require("../core/units/OrganizationalStructureManager");
const OrganizationalCompositionManager_1 = require("../core/units/OrganizationalCompositionManager");
const UnitDeploymentManager_1 = require("../core/units/UnitDeploymentManager");
...
class OrganizationalWorkflowManager {
    constructor(structureManager, compositionManager, deploymentManager, sharedStateManager, agentRegistry, taskManager) {
     ...
     */
    async addWorkflowStep(workflowId, name, description, unitId, agentIds, dependsOn = []) {
    ...
    async checkWorkflowCompletion(workflowId) {
        ...
        const allStepsCompleted = workflow.steps.every(step => step.status === 'completed' || step.status === 'failed');
...
exports.OrganizationalWorkflowManager = OrganizationalWorkflowManager;
/**
...
async function runStrategicInnovationWorkflow() {
    ...
    const sharedStateManager = new SharedStateManager_1.SharedStateManager();
    // Create the coordinator with the required components
    const coordinator = new MultiAgentCoordinator_1.MultiAgentCoordinator('StrategicInnovationWorkflow', {
        agentRegistry,
        taskManager,
        sharedStateManager
    });
    // Initialize the organizational managers
    const structureManager = new OrganizationalStructureManager_1.DefaultOrganizationalStructureManager();
    const compositionManager = new OrganizationalCompositionManager_1.DefaultOrganizationalCompositionManager(structureManager);
    const deploymentManager = new UnitDeploymentManager_1.UnitDeploymentManager(structureManager, compositionManager, agentRegistry, coordinator, taskManager);
    // Initialize the workflow manager
    const workflowManager = new OrganizationalWorkflowManager(structureManager, compositionManager, deploymentManager, sharedStateManager, agentRegistry, taskManager);
    // Load organizational unit definitions from the units/ directory
    const unitsDir = path_1.default.join(process.cwd(), 'units');
    ...
    const unitIds = {};
    const unitDirectories = fs_1.default.readdirSync(unitsDir, { withFileTypes: true })
    ...
    for (const unitName of requiredUnits) {
        if (unitDirectories.includes(unitName)) {
            const unitMdPath = path_1.default.join(unitsDir, unitName, `${unitName.toLowerCase()}_unit.md`);
            let description = `${unitName.replace(/_/g, ' ')} Unit`;
            // Try to extract description from markdown if available
            if (fs_1.default.existsSync(unitMdPath)) {
                const content = fs_1.default.readFileSync(unitMdPath, 'utf8');
    ...
    await structureManager.createUnitRelationship(unitIds['Strategy'], unitIds['Risk_Management'], 'service', 'Strategy provides plans to Risk Management for assessment');
    // Risk Management consults with Risk Advisory Board
    await structureManager.createUnitRelationship(unitIds['Risk_Management'], unitIds['Risk_Advisory_Board'], 'advisory', 'Risk Management consults with Risk Advisory Board');
    // Risk Management informs Innovation
    await structureManager.createUnitRelationship(unitIds['Risk_Management'], unitIds['Innovation'], 'service', 'Risk Management provides risk assessment to Innovation');
    // Innovation collaborates with Knowledge Management
    await structureManager.createUnitRelationship(unitIds['Innovation'], unitIds['Knowledge_Management'], 'collaboration', 'Innovation collaborates with Knowledge Management on implementation');
    // Create a composition of these units
    console.log("Creating organizational composition...");
    const compositionId = await compositionManager.createComposition('Knowledge Engineering Innovation Project', 'Strategic innovation project for knowledge engineering with risk assessment', Object.values(unitIds));
    ...
    const workflowId = await workflowManager.createWorkflow('Knowledge Engineering Innovation Project', 'Strategic innovation project for knowledge engineering with risk assessment', { compositionId });
    // Add workflow steps
    console.log("Adding workflow steps...");
    // Step 1: Strategy Planning
    const strategyStepId = await workflowManager.addWorkflowStep(workflowId, 'Strategy Planning', 'Develop the strategic plan for the knowledge engineering innovation project', unitIds['Strategy'], [agentIds['strategy_planner'], agentIds['strategy_analyst']], []);
    // Step 2: Risk Assessment
    const riskStepId = await workflowManager.addWorkflowStep(workflowId, 'Risk Assessment', 'Assess risks associated with the strategic plan', unitIds['Risk_Management'], [agentIds['risk_assessor'], agentIds['risk_mitigator']], [strategyStepId]);
    // Step 3: Risk Advisory Review
    const advisoryStepId = await workflowManager.addWorkflowStep(workflowId, 'Risk Advisory Review', 'Provide advisory input on the risk assessment', unitIds['Risk_Advisory_Board'], [agentIds['risk_advisor'], agentIds['risk_governance']], [riskStepId]);
    // Step 4: Innovation Planning
    const innovationStepId = await workflowManager.addWorkflowStep(workflowId, 'Innovation Planning', 'Develop innovation approaches based on strategy and risk assessment', unitIds['Innovation'], [agentIds['innovation_specialist'], agentIds['innovation_implementer']], [advisoryStepId]);
    // Step 5: Knowledge Engineering Implementation
    const knowledgeStepId = await workflowManager.addWorkflowStep(workflowId, 'Knowledge Engineering Implementation', 'Implement knowledge engineering solutions based on innovation plan', unitIds['Knowledge_Management'], [agentIds['knowledge_engineer'], agentIds['knowledge_curator']], [innovationStepId]);
    // Start the workflow
    ...
    console.log(strategyStep?.output.objectives.map((o) => `- ${o}`).join('\n'));
    console.log("\nKey Risks:");
    console.log(riskStep?.output.keyRisks.map((r) => `- ${r}`).join('\n'));
    console.log("\nRisk Advisory Recommendations:");
    console.log(advisoryStep?.output.recommendations.map((r) => `- ${r}`).join('\n'));
...
//# sourceMappingURL=strategic-risk-innovation-workflow.js.map