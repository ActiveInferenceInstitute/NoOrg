...
exports.runOrganizationalDemo = runOrganizationalDemo;
...
async function runOrganizationalDemo() {
    ...
    const coordinator = new MultiAgentCoordinator_1.MultiAgentCoordinator('OrganizationalDemo', {
        agentRegistry,
        taskManager,
        sharedStateManager
    });
    ...
    const compositionId = await createProjectComposition(structureManager, compositionManager, company);
    // Get the composition
 ...
 */
async function createSampleOrganization(structureManager) {
    ...
    await structureManager.createUnitRelationship(frontendTeamId, productManagementTeamId, 'collaboration', 'Frontend team collaborates with product management on UI/UX decisions');
    await structureManager.createUnitRelationship(backendTeamId, frontendTeamId, 'service', 'Backend team provides API services to the frontend team');
    await structureManager.createUnitRelationship(aiTeamId, backendTeamId, 'service', 'AI team provides ML services to the backend team');
    await structureManager.createUnitRelationship(marketingId, productId, 'collaboration', 'Marketing collaborates with product on messaging and positioning');
 ...
 */
async function displayOrganizationalStructure(structureManager, rootId, level = 0) {
 ...
 */
async function createProjectComposition(structureManager, compositionManager, company) {
    // Create a composition for a specific project
    const compositionId = await compositionManager.createComposition('Project Alpha', 'A project to develop a new AI-powered product', [company.teams.ai.id, company.teams.frontend.id, company.teams.backend.id, company.teams.productManagement.id]);
...
//# sourceMappingURL=organization-demo.js.map