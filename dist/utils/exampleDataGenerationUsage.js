...
const DataGenerationManager_1 = require("./DataGenerationManager");
 ...
 */
async function runWithContext(context, outputDir) {
    // Create a data generator with this context
    const dataGenerator = new DataGenerationManager_1.DataGenerationManager(process.env.OPENAI_API_KEY || '', context, path.join(outputDir, context.DOMAIN.replace(/\s+/g, '-').toLowerCase()));
    ...
    if (locations.length > 0) {
        ...
        const readings = await dataGenerator.generateDomainSpecificReadings(locations[0].name, new Date().toISOString());
        console.log('Generated readings:');
        console.log(readings);
    }
    // 3. Generate complete neural/sensor data
    console.log(`\nGenerating complete ${context.DOMAIN} data...`);
    const completeData = await dataGenerator.generateNeuralData(locations.map(loc => ({ name: loc.name, lat: loc.lat, lng: loc.lng })));
    ...
    const visConfig = await dataGenerator.generateVisualizationConfig(completeData, 'chart');
    ...
    fs.writeFileSync(path.join(contextDir, 'locations.json'), JSON.stringify(locations, null, 2));
    fs.writeFileSync(path.join(contextDir, 'complete-data.json'), JSON.stringify(completeData, null, 2));
    fs.writeFileSync(path.join(contextDir, 'visualization-config.json'), JSON.stringify(visConfig, null, 2));
...
//# sourceMappingURL=exampleDataGenerationUsage.js.map