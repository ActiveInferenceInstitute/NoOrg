...
describe('EventSystem', () => {
  ...
  describe('Event Publishing and Subscription', () => {
    ...
    it('should handle wildcard topic subscriptions', (done) => {
      ...
      const subscriptionId = eventSystem.subscribe(`${baseTopic}.*`, (event) => {
        expect(event.topic).to.equal(specificTopic);
        expect(event.data).to.deep.equal(testData);
        done();
      });
  ...
  describe('Subscription Management', () => {
    ...
    it('should provide subscription management methods', () => {
      ...
      expect(eventSystem.getSubscriptionCount()).to.be.at.least(1);

      eventSystem.unsubscribe(subscriptionId);
      expect(eventSystem.getSubscriptionCount()).to.be.below(eventSystem.getSubscriptionCount() + 1);
    });
  });
  ...
  describe('Event Metadata', () => {
    it('should include proper metadata in events', (done) => {
      ...
      const subscriptionId = eventSystem.subscribe(testTopic, (event) => {
        expect(event.metadata).to.have.property('timestamp');
        expect(event.metadata).to.have.property('source');
        expect(event.metadata!.source).to.equal(source);
        expect(event.metadata!.timestamp).to.be.a('number');
        expect(event.metadata!.timestamp).to.be.at.most(Date.now());
        done();
      });
    ...
    it('should support correlation IDs', (done) => {
      ...
      const subscriptionId = eventSystem.subscribe(testTopic, (event) => {
        ...
        expect(event.metadata!.correlationId).to.equal(correlationId);
        done();
      });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle malformed event data', () => {
      ...
      expect(() => {
        eventSystem.publish(testTopic, null as any);
      }).to.not.throw();

      expect(() => {
        eventSystem.publish(testTopic, undefined as any);
      }).to.not.throw();
    });
  });
  ...
  describe('Performance', () => {
    it('should handle high-frequency events', async () => {
      ...
      expect(endTime - startTime).to.be.below(1000); // Should complete in less than 1 second
    });
    ...
    it('should handle many subscribers efficiently', async () => {
      ...
      expect(totalReceived).to.equal(subscriberCount);
    });
  });
