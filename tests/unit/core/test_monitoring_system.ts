...
describe('MonitoringSystem', () => {
  ...
  describe('Metrics Collection', () => {
    it('should record and retrieve metrics', () => {
      ...
      if (typeof monitoring.getMetric === 'function') {
        ...
        expect(retrieved).to.be.at.least(metricValue);
      }
    });
  ...
  describe('Health Checks', () => {
    it('should register and execute health checks', async () => {
      ...
      expect(result.message).to.equal('Test health check passed');
    ...
    it('should handle failing health checks', async () => {
      ...
      expect(result.message).to.equal('Test health check failed');
      expect(result.details).to.have.property('error');
    });
    ...
    it('should unregister health checks', async () => {
      ...
      expect(result1.status).to.equal('healthy');
  ...
  describe('Alert Rules', () => {
    it('should create and manage alert rules', () => {
      ...
      if (typeof monitoring.getAlertRule === 'function') {
        ...
        expect(rule.name).to.equal('test-alert-rule');
    ...
    it('should remove alert rules', () => {
      ...
      if (typeof monitoring.getAlertRule === 'function') {
        try {
          ...
          expect.fail('Rule should have been removed');
        } catch (error: any) {
          expect(error.message).to.include('not found');
        }
      }
  ...
  describe('System Health Overview', () => {
    it('should provide overall system health', async () => {
      ...
      if (typeof monitoring.getOverallHealth === 'function') {
        ...
        expect(overallHealth).to.have.property('status');
        expect(overallHealth).to.have.property('timestamp');
      }
    });
  });
  ...
  describe('Alert Processing', () => {
    ...
    it('should support multiple notification channels', () => {
      ...
      if (typeof monitoring.getAlertRule === 'function') {
        const rule = monitoring.getAlertRule(ruleId);
        expect(rule.notificationChannels).to.include('email');
        expect(rule.notificationChannels).to.include('slack');
        expect(rule.notificationChannels).to.include('webhook');
      }
    });
  });
  ...
  describe('Performance Monitoring', () => {
    it('should track performance metrics', () => {
      ...
      expect(duration).to.be.below(1000); // Should complete quickly
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle health check errors', async () => {
      ...
      try {
        ...
        expect(error.message).to.equal('Health check error');
      }
    });
