import { Retry } from '../../../src/core/integration/patterns/Retry';
...
describe('Retry Pattern', () => {
  ...
  test('should respect retryable errors list', async () => {
    ...
    const retryWithFilters = new Retry('test-retry-filters', {
      maxAttempts: 3,
      initialDelay: 10,
  ...
  test('should use static getInstance method correctly', async () => {
    const instance1 = Retry.getRetrier('shared-retry');
    const instance2 = Retry.getRetrier('shared-retry');
