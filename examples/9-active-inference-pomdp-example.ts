...
const PROJECT_CONFIG = {
  STORY: `
    The stainless steel kitchen counter gleamed under the morning sun as Chef Marco prepared 
    his workstation. A fresh egg sat in a ceramic bowl at room temperature, its surface 
    slightly dewed from the cool 18°C air circulating through the climate-controlled kitchen. 
    The digital thermometer above the induction cooktop blinked green, indicating optimal 
    conditions for ingredient preparation.
    
    At midday, sunlight streamed through the west-facing windows as the kitchen's temperature 
    began to rise. The climate control system struggled against the heat generated by three 
    active ovens baking artisanal bread. The mercury in the analog thermometer mounted near 
    the ventilation hood climbed steadily to 32°C. Marco observed the egg's proteins just 
    starting to coagulate at the edges, a delicate white forming around the vibrant yolk.
    
    By mid-afternoon, the kitchen transformed into a sauna-like environment. The temperature 
    display flashed 45°C as the combination of oven heat, steam from boiling pots, and 
    malfunctioning AC took its toll. The egg sizzled on an overheated marble surface, its 
    albumen fully cooked into crisp golden lace while the yolk thickened into a creamy custard-like 
    consistency. Condensation dripped down the windows as Marco adjusted his chef's jacket and 
    documented the thermal transformation.
  `,
