...
import { AgentDiscoveryService, AgentInfo } from '../discovery/AgentDiscoveryService';
import { CircuitBreaker, Retry } from '../../integration/patterns';
...
export class AgentOrchestrator {
  ...
  private taskQueue: string[]; // Ordered task IDs by priority
  private taskAssignmentInterval?: NodeJS.Timeout;
  private agentCircuitBreakers: Map<string, CircuitBreaker>;
  private taskRetry: Retry;
  private options: Required<OrchestratorOptions>;

  private constructor(options: OrchestratorOptions = {}) {
    this.eventSystem = EventSystem.getInstance();
    this.storageSystem = StorageSystem.getInstance();
    this.discoveryService = AgentDiscoveryService.getInstance();
    this.tasks = new Map();
    this.taskQueue = [];
    ...
    this.taskRetry = new Retry('task-orchestrator', {
   ...
   */
  public async submitTask(task: Omit<TaskDefinition, 'createTime' | 'status'>): Promise<TaskDefinition> {
   ...
   */
  public getTaskStatus(id: string): TaskDefinition | undefined {
   ...
   */
  public getTasksByStatus(status: TaskDefinition['status']): TaskDefinition[] {
   ...
   */
  private startTaskAssignment(): void {
   ...
   */
  private selectBestAgent(agents: AgentInfo[], task: TaskDefinition): AgentInfo | null {
   ...
   */
  private async assignTaskToAgent(task: TaskDefinition, agent: AgentInfo): Promise<boolean> {
    // Update task status
  ...
  private getCircuitBreaker(agentId: string): CircuitBreaker {
    if (!this.agentCircuitBreakers.has(agentId)) {
      const breaker = new CircuitBreaker(`agent-${agentId}`, {
   ...
   */
  private async handleTaskStarted(taskId: string, agentId: string): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task || task.assignedTo !== agentId) return;
   ...
   */
  private async handleTaskCompleted(taskId: string, agentId: string, result: any): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task || task.assignedTo !== agentId) return;
   ...
   */
  private async handleTaskFailed(taskId: string, agentId: string, error: string): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task || task.assignedTo !== agentId) return;
   ...
   */
  private async handleAgentExpired(agentId: string): Promise<void> {
  ...
  private async loadTasksFromStorage(): Promise<void> {
    try {
      ...
      const taskRegistry = await this.storageSystem.get<string[]>('task:registry') || [];
      
      for (const taskId of taskRegistry) {
        const task = await this.storageSystem.get<TaskDefinition>(`task:${taskId}`);
        if (task) {
