...
import { OrganizationalUnit, UnitWorkflow, UnitCollaboration } from '../UnitInterface';
import { UnitStateManager } from '../state/UnitStateManager';
import { OpenAIClient } from '../../multiagent/OpenAIClient';
import { UnitAgentFactory } from '../agents/UnitAgentFactory';
...
export class TaskOrchestrator {
  ...
  private tasks: Map<string, UnitTask>;
  private stateManager: UnitStateManager;
  private openaiClient?: OpenAIClient;
  private agentFactory: UnitAgentFactory;
  private unitAgentMap: Map<string, string>; // Maps unit IDs to agent IDs
  ...
  constructor(
    ...
    agents: Agent[],
    workflow: UnitWorkflow | undefined,
    ...
    if (this.config.openaiApiKey) {
      this.openaiClient = new OpenAIClient(this.config.openaiApiKey);
    } else {
      this.logger.warn('No OpenAI API key provided. LLM-based execution will be unavailable.');
    }
    
    // Initialize agent factory
    this.agentFactory = new UnitAgentFactory();
   ...
   */
  public createTask(taskData: Partial<UnitTask>): UnitTask {
   ...
   */
  public assignTask(taskId: string, unitId: string): UnitTask {
   ...
   * @returns Updated task
   */
  public updateTask(taskId: string, updates: Partial<UnitTask>): UnitTask {
    const task = this.getTask(taskId);
   ...
   * @returns The task
   */
  public getTask(taskId: string): UnitTask {
   ...
   */
  public areDependenciesMet(taskId: string): boolean {
    const task = this.getTask(taskId);
  ...
  private async executePlanningPhase(
    ...
    if (!this.workflow?.planningPhase) return;
    ...
    const collaborationTasks: UnitTask[] = [];
    
    for (const unitId of phase.participatingUnitIds) {
      // Skip the lead unit
      if (unitId === phase.leadUnitId) continue;
    ...
    const collabResults = await Promise.all(
      collaborationTasks.map(task => this.executeTask(task.id))
    );
   ...
   */
  private async executeExecutionPhase(topic: string): Promise<void> {
    if (!this.workflow?.executionPhase) return;
    ...
    const executionResults = await Promise.all(
      executionTasks.map(task => this.executeTask(task.id))
    );
   ...
   */
  private generateTaskPrompt(task: UnitTask, unit: OrganizationalUnit): string {
...
`;
   ...
   * @param agents - Array of agents
   */
  private buildUnitAgentMap(agents: Agent[]): void {
