...
import { Workflow, WorkflowTask, TaskStatus } from '../../core/units/workflow/WorkflowEngine';
 ...
 */
export function generateWorkflowSummary(workflow: Workflow): string {
  ...
  const skipped = workflow.tasks.filter(t => t.status === TaskStatus.SKIPPED).length;
  const running = workflow.tasks.filter(t => t.status === TaskStatus.RUNNING).length;
  const pending = workflow.tasks.filter(t => t.status === TaskStatus.PENDING).length;
  const cancelled = workflow.tasks.filter(t => t.status === TaskStatus.CANCELLED).length;
  
  let totalDuration = 0;
  let maxTaskDuration = 0;
  let minTaskDuration = Infinity;
  let longestTask: WorkflowTask | null = null;
  let shortestTask: WorkflowTask | null = null;
 ...
 */
export function generateWorkflowHTML(workflow: Workflow): string {
  ...
  <div class="container">
    ...
    <div class="stats">
      <div class="stat-card">
        ...
        <p>Completed: ${workflow.tasks.filter(t => t.status === TaskStatus.COMPLETED).length}</p>
        <p>Failed: ${workflow.tasks.filter(t => t.status === TaskStatus.FAILED).length}</p>
        <p>Skipped: ${workflow.tasks.filter(t => t.status === TaskStatus.SKIPPED).length}</p>
        <p>Running: ${workflow.tasks.filter(t => t.status === TaskStatus.RUNNING).length}</p>
        <p>Pending: ${workflow.tasks.filter(t => t.status === TaskStatus.PENDING).length}</p>
        <p>Cancelled: ${workflow.tasks.filter(t => t.status === TaskStatus.CANCELLED).length}</p>
      </div>
      ...
      <div class="stat-card">
        ...
        ${(() => {
          const completedTasks = workflow.tasks.filter(t => t.status === TaskStatus.COMPLETED && t.startTime && t.endTime);
          if (completedTasks.length === 0) return '<p>No completed tasks yet</p>';
