...
// Define options specific to CreativeWritingAgent
// Allow 'capabilities' to be passed optionally
interface CreativeWritingAgentOptions extends Omit<AbstractAgentOptions, 'type'> {
    cacheTTL?: number; // Cache time-to-live in ms (default 3 hours)
}
...
export class CreativeWritingAgent extends AbstractAgent {
        ...
        try {
            ...
            switch (taskDetails?.type) {
                ...
                case 'generateContent': // Allow alias
                    if (!taskDetails.prompt) throw new Error('Missing required field "prompt" for generate task.');
                    result = await this.generateContent(taskDetails.prompt, taskDetails.options);
                    break;
                case 'refine':
                case 'refineContent': // Allow alias
                     if (!taskDetails.content) throw new Error('Missing required field "content" for refine task.');
                     if (!taskDetails.feedback) throw new Error('Missing required field "feedback" for refine task.');
                    result = await this.refineContent(taskDetails.content, taskDetails.feedback);
                    break;
                case 'generateStylized':
                     if (!taskDetails.prompt) throw new Error('Missing required field "prompt" for generateStylized task.');
                     if (!taskDetails.style) throw new Error('Missing required field "style" for generateStylized task.');
     ...
     */
    async generateContent(contentPrompt: string, options?: {
        ...
        try {
            ...
            if (length === 'short') lengthInstruction = 'Create a concise piece around 150-250 words.';
            else if (length === 'medium') lengthInstruction = 'Write a balanced piece of approximately 400-600 words.';
            else lengthInstruction = 'Develop an in-depth piece of approximately 800-1200 words.';
            
            let formatInstruction = '';
            if (format === 'story') formatInstruction = 'Create an engaging narrative with characters, plot, and setting.';
            else if (format === 'blog-post') formatInstruction = 'Write an informative blog post with headings, paragraphs, and a clear structure. Start with a title line like "# Title". Use section headings like "## Section Title".';
            else if (format === 'marketing-copy') formatInstruction = 'Develop persuasive marketing copy that highlights benefits and includes a call to action.';
            else if (format === 'poem') formatInstruction = 'Compose a poem with appropriate structure, rhythm, and poetic devices.';
            else if (format === 'dialogue') formatInstruction = 'Create a realistic dialogue between characters that advances a narrative or makes a point.';
            else formatInstruction = 'Write content in any appropriate format for the topic.';
            ...
            if (format === 'blog-post') {
                ...
                sections = [];
                let currentSectionTitle: string | null = title || 'Introduction'; // Default if no # title
     ...
     */
    async refineContent(originalContent: string, feedback: string): Promise<{
     ...
     */
    async generateStylizedContent(contentPrompt: string, style: string, options?: {
        format?: 'story' | 'blog-post' | 'marketing-copy' | 'poem' | 'dialogue' | 'free-form';
        length?: 'short' | 'medium' | 'long';
        examples?: string[]; // Examples of the target style
        temperature?: number;
    }): Promise<{
        content: string;
        styleConfidence: number; // Model's confidence in matching the style (heuristic)
        processingTime: number;
    }> {
        const startTime = Date.now();
        ...
        try {
            ...
            let lengthInstruction = '';
            if (length === 'short') lengthInstruction = 'Keep it concise (150-250 words).';
            else if (length === 'medium') lengthInstruction = 'Write approx 400-600 words.';
            else lengthInstruction = 'Write approx 800-1200 words.';
    ...
    // Removed initialize, updateStatus, getAgentInfo, shutdown (handled by AbstractAgent)
} 