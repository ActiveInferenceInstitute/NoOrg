...
Coverage is enforced at 90% lines and 80% branches by Jest and is part of the ordinary `npm run validate` gate (the coverage-augmented test run), so a coverage regression fails the same gate as a failing test. The coverage command remains local and deterministic; live provider checks are never part of the ordinary validation command.
