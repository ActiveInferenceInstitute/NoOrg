...
Provider credentials are read only at application startup and are never included in task results, events, metrics, or logs. Task and metrics HTTP endpoints require a bearer token whenever configured, and production startup requires one. Provision checks use constant-time comparison, failed authentication is throttled per client, and the server bounds concurrent in-flight requests and request timeouts. Prompt and state paths are validated and runtime writes use atomic replacement.
